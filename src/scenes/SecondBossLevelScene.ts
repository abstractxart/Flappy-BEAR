...
/**
 * Second Boss Level Scene
 */
export class SecondBossLevelScene extends Phaser.Scene {
  // Game objects
  bear!: FlappyBear;
  boss!: SecondBoss;
  ...
  missileKey!: Phaser.Input.Keyboard.Key;
  ...
  missileCooldown: number = 800; // 0.8 seconds between missiles
  
  // Music
  private musicManager: MusicManager;

  constructor() {
    super({ key: "SecondBossLevelScene" });
    this.musicManager = MusicManager.getInstance();
  }

  create(): void {
    // FORCE RESET all scene state for fresh boss fight
    console.log("🔄 SECOND BOSS SCENE CREATE - RESETTING STATE");
    ...
    // Create background (same as title screen for consistency)
    ...
    console.log("Second Boss Level Scene setup complete");
  }
  ...
  setupCollisions(): void {
    // Bear vs boss projectiles
    utils.addOverlap(
      this,
      this.bear,
      this.boss.projectilesGroup,
  ...
  setupBossEvents(): void {
    ...
    this.events.on('secondBossDefeated', () => {
      this.handleBossDefeated();
    });
    
    // Listen for bear death event
    this.events.on('bearDied', () => {
      console.log("SecondBossLevelScene received bearDied event");
      if (!this.gameOver) {
  ...
  startGame(): void {
    ...
    console.log("Second Boss fight started!");
  }
  ...
  handleProjectileHit(bear: FlappyBear, projectile: any): void {
    if (bear.isInvulnerable || this.gameOver) return;
    
    // Remove projectile
    projectile.destroy();
    
    console.log("Bear hit by projectile, health before damage:", bear.health);
    
    // Damage bear (bear.takeDamage will handle death and game over)
    bear.takeDamage(1);
    
    console.log("Bear health after projectile damage:", bear.health, "isDead:", bear.isDead, "gameOver:", this.gameOver);
  }

  handleMissileHit(missile: Missile, boss: SecondBoss): void {
    console.log("Missile hit second boss at:", missile.x, missile.y, "Boss at:", boss.x, boss.y);
    
    // Explode missile
    missile.explode();
    
    // Try to damage boss (same 25 damage as first boss)
    ...
    if (damageDealt) {
      ...
      // Show visual feedback for invalid hit (shouldn't happen with second boss)
  ...
  handleBossContact(bear: FlappyBear, boss: SecondBoss): void {
    if (bear.isInvulnerable || this.gameOver) return;
    
    // Push bear away
    bear.body.setVelocityX(-200);
    
    console.log("Bear contacted second boss, health before damage:", bear.health);
    
    // Damage bear (bear.takeDamage will handle death and game over)
    bear.takeDamage(1);
    
    console.log("Bear health after second boss contact damage:", bear.health, "isDead:", bear.isDead, "gameOver:", this.gameOver);
  }
  ...
  handleBossDefeated(): void {
    ...
    console.log("Second Boss defeated! Victory!");
    ...
    // Add 200+ bonus points for defeating second boss
    const bonusPoints = 300; // Higher bonus for second boss
    ...
    console.log(`Second Boss victory! +${bonusPoints} points, speed now ${newSpeedMultiplier}x`);
  ...
  update(time: number, delta: number): void {
    ...
    if (this.bear.y > this.scale.height + 100 && !this.gameOver) {
      console.log("Bear fell off screen in second boss");
      this.gameOver = true;
      this.handleGameOver();
    }
  }
  ...
  setupPauseResumeHandlers(): void {
    console.log("🎮 Setting up pause/resume handlers for second boss scene");
    
    // Handle scene pause event
    this.events.on('pause', () => {
      console.log("⏸️ SECOND BOSS SCENE PAUSED");
    ...
    this.events.on('resume', () => {
      console.log("▶️ SECOND BOSS SCENE RESUMED");
  ...
  shutdown(): void {
    console.log("🧹 SECOND BOSS SCENE SHUTDOWN - Cleaning up");
